...
- This walkthrough documents an end-to-end Land Use Regression (LUR) workflow built to predict PM~2.5~ at high spatial resolution using remote-sensing data. 
- It shows how to identify and work with different data types like raster (e.g. satellite AOD, elevation), vector (site locations, buffers, railway lines) and tabular/CSV data and save each dataset in the correct format (GeoTIFF, shapefile/GeoPackage, CSV, RDS). 
- It covers extracting OpenStreetMap data for roads, railways, and airport; interpolating pollutant surfaces at 50m resolution using GIS software; and collating predictors from multiple sources (Earth Engine, OSM, ground monitoring) into one clean, merged dataset using a unique site ID. 
- The model-building steps walk through univariate screening, stepwise variable selection, multicollinearity (VIF) and influence (Cook's distance) checks, and leave-one-out cross-validation, producing a validated high-resolution prediction map using the final equation. 
- This walk through is avialble on GitHub so that public health practitioners and researchers with / without coding experience can follow, reproduce, and adapt each step themselves.


:::{.callout-note}
A few datasets used here - Land use land cover, NDVI, population have already been extracted and saved as csv files for the sites of the air pollution monitors across Delhi.
:::
...
delhi_sites_proj %>% 
  dplyr::select(everything(), -c(geometry)) %>% 
  head() %>% 
  gt() %>% 
  tab_options(
...
- Response variable: PM~2.5~

- Missing values: 5

::: {.cell-output-display style="overflow-x:auto;"}
| Dataset and reference | Predictor description | Unit | Buffer size (m) | Direction of effect |
|:----------------------|:----------------------|:----:|:---------------:|:-------------------:|
| **Land-use Land-cover**<br>(ESA WorldCover v100, 2020 product)<br>Zanaga et al. (2021) | Surface area of built-up area | m² | 100, 300, 500, 1000, 5000 | + |
| | Surface area of permanent water bodies | m² | 100, 300, 500, 1000, 5000 | − |
| | Surface area of tree cover | m² | 100, 300, 500, 1000, 5000 | − |
| | Surface area of bare/sparse vegetation | m² | 100, 300, 500, 1000, 5000 | + / − |
| | Surface area of cropland | m² | 100, 300, 500, 1000, 5000 | − |
| | Surface area of scrubland/shrubland, grassland, and herbaceous wetland | m² | 100, 300, 500, 1000, 5000 | − |
| **Population**<br>(GHSL, 2015 product)<br>Freire et al. (2020) | Number of inhabitants | # | 300, 500, 1000, 5000 | + |
| **Elevation (SRTM)**<br>Farr et al. (2007) | Square root of elevation | m<sup>1/2</sup> | NA | − |
| **Geographical variables** | Longitude | °E | NA | + / − |
| | Latitude | °N | NA | + / − |
| **Railway network (OSM)** | Length of railway line | m | 500, 1000, 5000 | + |
| **Airport location** | Inverse distance to the runway | m⁻¹ | NA | + |
| **Industry location** | Inverse distance to the nearest industry; inverse square distance to the nearest industry | m⁻¹; m⁻² | NA | + |
| **NDVI**<br>(Sentinel-2, 2019 product)<br>Gascon et al. (2016) | Normalized Difference Vegetation Index (NDVI) | – | 100, 300, 500, 1000, 5000 | − |
| **NTLI**<br>(VIIRS, 2019 product)<br>Elvidge et al. (2017) | Night-time light intensity | – | NA | + |
| **Satellite NO₂ (TROPOMI)** | Tropospheric columnar NO₂ | molecules m⁻² | NA | + |
| **AOD**<br>(MODIS-MAIAC, 2019 product)<br>Lyapustin et al. (2018) | Aerosol Optical Depth | – | NA | + |
| **Meteorological data**<br>(ERA5-Land, 2019 product)<br>Muñoz-Sabater et al. (2021) | 2 m air temperature | K | NA | − |
| | 2 m pressure | Pa | NA | + |
| | 10 m eastward component of wind | m s⁻¹ | NA | − |
| | 10 m northward component of wind | m s⁻¹ | NA | − |
| | 10 m resultant wind speed | m s⁻¹ | NA | − |
| | Total precipitation | m | NA | − |
:::
...
ntli_name <- "Delhi_NTLI_Jan2019_Dec2019.tif"
no2_name <- "Delhi_Tropospheric_NO2_Sentinel5P_2019.tif"
ws_name <- "Delhi_windspeed_2019.tif"
uw_name <- "Delhi_u_wind_2019.tif"
vw_name <- "Delhi_v_wind_2019.tif"
temp_name <- "Delhi_Temperature_2019.tif"
prec_name <- "Delhi_TotalPrecipitation_2019.tif"
pres_name <- "Delhi_Pressure_2019.tif"

aod <- raster(here("data/AOD/interpolated", aod_name))
dem <- raster(here("data/DEM", dem_name))
ntli <- raster(here("data/NTLI", ntli_name))
no2 <- raster(here("data/NO2/interpolated", no2_name))
ws <- raster(here("data/Met/interpolated", ws_name))
u_wind <- raster(here("data/Met/interpolated", uw_name))
v_wind <- raster(here("data/Met/interpolated", vw_name))
temperature_2m <- raster(here("data/Met/interpolated", temp_name))
precipitation <- raster(here("data/Met/interpolated", prec_name))
pressure <- raster(here("data/Met/interpolated", pres_name))
